...
export function AgencyRankingView() {
  ...
  const ranking = useMemo(() => clientRanking(sales), [sales]);

  const totalValue = ranking.reduce((s, r) => s + r.totalValue, 0);
  const totalSales = ranking.reduce((s, r) => s + r.salesCount, 0);
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="text-xs text-gray-400">
          {ranking.length} cliente{ranking.length !== 1 ? 's' : ''} | {totalSales} vendas | {formatCurrency(totalValue)}
        </div>
      </div>
            ...
            <tbody>
              {ranking.map((r, i) => (
                <tr key={r.client} className="border-t border-navy-700 hover:bg-navy-700/50 transition-colors">
                  ...
                  <td className="px-5 py-2.5 text-center text-gray-400">{r.salesCount}</td>
