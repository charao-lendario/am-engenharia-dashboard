...
import { agencyRanking } from '../../utils/dataTransformers';
...
export function AgencyRankingView() {
  ...
  const ranking = useMemo(() => agencyRanking(contracts), [contracts]);

  const totalBrokered = ranking.reduce((s, r) => s + r.totalValue, 0);
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="text-xs text-gray-400">
          {ranking.length} canal{ranking.length !== 1 ? 'is' : ''} | {totalContracts} contratos | {formatCurrency(totalBrokered)}
        </div>
      </div>
              ...
              {ranking.map((r, i) => (
                <tr key={r.broker} className="border-t border-navy-700 hover:bg-navy-700/50 transition-colors">
                  ...
                  <td className="px-5 py-2.5 text-right text-gray-400">
                    {totalBrokered > 0 ? ((r.totalValue / totalBrokered) * 100).toFixed(1) : 0}%
                  </td>
                </tr>
              ))}
            </tbody>
